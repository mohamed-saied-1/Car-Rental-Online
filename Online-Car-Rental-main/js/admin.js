...
async function loadLogs() {
    const content = document.getElementById('content-area');
    content.innerHTML = '<div class="loading">Fetching Security Audit Logs...</div>';
    ...
    try {
        ...
        content.innerHTML = `
            ...
            <div class="table-section">
                <table class="logs-table">
                    ...
                    <tbody>
                        ${data.logs.map(log => {
                            // نظام الألوان بناءً على الخطورة (Security Levels)
                            const typeMap = {
                                'info': { color: '#3498db', icon: 'ri-information-line' },
                                'success': { color: '#2ecc71', icon: 'ri-checkbox-circle-line' },
                                'warning': { color: '#f1c40f', icon: 'ri-error-warning-line' }, // محاولة فاشلة
                                'danger': { color: '#e74c3c', icon: 'ri-alert-line' }    // حظر Brute Force
                            };
                            const theme = typeMap[log.type] || typeMap['info'];
                            
                            // تحديد لون حالة العملية (Success / Failed / Blocked)
                            const statusColor = log.status === 'failed' ? '#e74c3c' : (log.status === 'blocked' ? '#f39c12' : '#2ecc71');
                            ...
                            return `
                                <tr style="border-left: 5px solid ${theme.color};">
                                    ...
                                    <td>
                                        <strong>${log.action.toUpperCase()}</strong><br>
                                        <span style="color: ${statusColor}; font-weight: bold; font-size: 0.75rem;">
                                            ● ${log.status.toUpperCase()}
                                        </span>
                                    </td>
                                    <td style="max-width: 250px;">
                                        ${log.message}
                                        <br><small style="color: ${theme.color};"><i class="${theme.icon}"></i> ${log.type.toUpperCase()}</small>
                                    </td>
                                    <td style="font-family: monospace; font-size: 0.8rem; min-width: 200px;">
                                        <div style="background: #f8f9fa; padding: 8px; border-radius: 4px; word-break: break-all; white-space: normal;">
                                            <strong>IP:</strong> ${log.ip_address || '127.0.0.1'}<br>
                                            <strong>Agent:</strong> ${log.user_agent || 'N/A'}
                                        </div>
                                    </td>
                                </tr>
                            `;
        ...
        content.innerHTML = '<p class="error">Failed to load system logs. Make sure the server is running.</p>';
    }
}
// --- VIEW: DASHBOARD ---
