...
// --- LOGIN FUNCTION (Security Version) ---
async function login() {
  clearMessages(); 
  const email = document.getElementById("loginEmail").value;
  const password = document.getElementById("loginPassword").value;

  console.log("Login attempt:", { email }); // شلنا الباسورد من الكونسول عشان السيكيورتي
  ...
  try {
    ...
    if (data.success) {
      // لاحظ هنا بنقرأ من data.user لأن ده التعديل اللي عملناه في السيرفر
      localStorage.setItem('userId', data.user.id);
      localStorage.setItem('userRole', data.user.role);
    
    // التعديل الضروري: استخدم الأسماء اللي جاية من الـ SQL بالظبط
      localStorage.setItem('userFirstName', data.user.firstName); 
    localStorage.setItem('userLastName', data.user.lastName);   
      localStorage.setItem('userEmail', data.user.email);
      localStorage.setItem('userPhone', data.user.phone);
    
    // تخزين الاسم بالكامل عشان صفحة الـ booking
      ...
      // هنا هيعرض رسائل الخطأ زي "Wrong password" أو "Account Locked"
